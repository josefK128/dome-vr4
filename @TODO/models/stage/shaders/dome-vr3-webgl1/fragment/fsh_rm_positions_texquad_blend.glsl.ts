...
// fsh_rm_positions_texquad - rm-quad NDC [-1,1]x[-1,1] texture map
...
void main( void ) {
    ...
    rmcolor=vec4(0.0,0.5,0.0,1.0);

    //texture map if no raymarch
//	if(color.rgb == vec3(0.0,0.0,0.0)){
//          gl_FragColor = texture2D(tDiffuse, vuv);  // needs vertex shader definition !  
//	}else{
//	  gl_FragColor = vec4(color, 1.0) + rmcolor;
//	}

      // texture2D is black!!! why?
	  gl_FragColor = texture2D(tDiffuse,vuv) + vec4(color, 1.0) + rmcolor;
